...
class AbstractGaussianNode(VariationalNode, ABC):
    ...
    def initialize(
        ...
        **kwargs: Any,
        ...
        if not self.shape.compat(*mu.shape):
            raise ValueError(
                f"shape generated by `self.prediction(*pred)` {(*mu.shape,)} "
                f"is incompatible with node shape {(*self.shape,)}"
            )

        self.value.data = self.value.data.new_empty(*mu.shape)
        self.value.copy_(mu)
...
class MultivariateGaussianNode(AbstractGaussianNode):
    ...
    def energy(self, *pred: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        p = self.value.numel() / (self.value.size(0) * n)  # number of features

        # quadratic
        z, pragma = self.shape.coalesce(self.value)
