...
- E-Step (Inference): $\mathbf{x}^\ast = \operatorname{arg\,min}_{\mathbf{x}} \mathcal{F}(\mathbf{x}, \boldsymbol{\Theta})$
- M-Step (Learning): $\boldsymbol{\Theta}^\ast = \operatorname{arg\,min}_{\boldsymbol{\Theta}} \mathcal{F}(\mathbf{x}, \boldsymbol{\Theta})$
